...
class TestTemperatureController(BaseTestCase):
    ...
    def test_get_last_measure_by_sensor_and_type(self):
        """Test case for get_last_measure_by_sensor_and_type

        Get last measure from sensor
        """
        response = self.client.open(
            '/v2/getHistoricalMeassuresBySensorAndType/{sensor}/{start}/{end}'.format(sensor='sensor_example', start='start_example', end='end_example'),
            method='GET',
            content_type='application/json')
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
